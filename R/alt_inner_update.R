...
#' Alternative function (trial for changes in algorithm)
#'
...
#'
#' @export
alt_inner_update <- function(x, data, yvar, index.vars, linear.vars, 
  ...
  while(maxIt <= max.iter){
    alpha_new <- alt_update_alpha(Y = R, X = X_new, num_pred = num_pred, 
