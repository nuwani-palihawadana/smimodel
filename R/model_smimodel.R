...
#' \sum_{k = 1}^{d}f_{k}(w_{ik}) + \bm{\theta}^{T}\bm{u}_{i} +
#' \varepsilon_{i}, \quad i = 1, \dots, n,} where \eqn{y_{i}} is the univariate
#' response, \eqn{\beta_{0}} is the model intercept, \eqn{\bm{x}_{ij} \in
#' \mathbb{R}^{l_{j}}}, \eqn{j = 1, \dots, p} are \eqn{p} subsets of predictors
#'   entering indices, \eqn{\bm{\alpha}_{j}} is a vector of index
#'   coefficients corresponding to the index \eqn{h_{ij} =
#'   \bm{\alpha}_{j}^{T}\bm{x}_{ij}}, and \eqn{g_{j}} is a
...
#'   \eqn{\bm{x}_1,\dots,\bm{x}_n}, with each vector
#'   \eqn{\bm{x}_i} containing \eqn{q} predictors. This function
